...
import {
    ...
    type RegistrarIngresoFormInput,
    type RegistrarIngresoFormValues,
} from '@/features/tesoreria/schemas/tesoreriaSchemas';
import { zodResolver } from '@hookform/resolvers/zod';
import { useForm } from 'react-hook-form';

type RegistrarIngresoFormProps = {
    onSuccess: () => void;
    onCancel: () => void;
};

const defaultValues: RegistrarIngresoFormInput = {
    monto: 0,
    concepto: '',
    cuentaContableId: '',
    cajaId: '',
...
export default function RegistrarIngresoForm({ onSuccess, onCancel }: RegistrarIngresoFormProps) {
    ...
    const {
        ...
        handleSubmit,
        formState: { errors },
    } = useForm<RegistrarIngresoFormInput, unknown, RegistrarIngresoFormValues>({
        resolver: zodResolver(registrarIngresoSchema),
        defaultValues,
    });

    const onSubmit = async (values: RegistrarIngresoFormValues) => {
        await registrarIngresoMutation.mutateAsync({
            monto: values.monto,
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
                ...
                >
                    {registrarIngresoMutation.isPending ? 'Registrando...' : 'Registrar ingreso'}
                </button>
            </div>
        </form>
    );
